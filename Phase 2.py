...
def add_obstacles(n=20, shape=SHAPE, start=STARTING_POINT, finish=DELIVERY_POINT, obst=OBSTACLES):
    new = [*obst]
    num = 0
    while True:
        (x, y) = (randint(0, shape[0]-1), randint(0, shape[1]-1))
        if (x, y) not in new and (x, y) not in (start, finish):
            num += 1
            new.append((x, y))
